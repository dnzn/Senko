namespace Senko
{
    ...
    public partial class SonyDevice
    {
        ...
        public partial class Action
        {
            public class Alias
            {
                static Regex RegexGeneratedJSON { get; } = new Regex(@"^({)(\s*""\w+"":\s*""\w+"",*\s*)+(})$");
                static Regex RegexAliasFile { get; } = new Regex(@"^((\w+)=(([\w, ])+)[\r\n]*)+$");
                ...
                public bool Parse(string file)
                {
                    ...
                    if (File.Exists(file))
                    {
                        ...
                        if (RegexGeneratedJSON.IsMatch(text)) // Match to JSON generated by this class
                        {
                            try
                            {
                        ...
                        else if (RegexAliasFile.IsMatch(text)) // Match to .ALIAS
                        {
                            int duplicates = 0;
