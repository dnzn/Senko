...
public static class Static
{
    ...
    public struct Encapsulator
    {
        public EncapsulatorType? Type { get; set; }
        public string Opening { get; set; }
        public string Closing { get; set; }

        public Encapsulator(EncapsulatorType type, string opening, string closing)
        {
            Type = type;
            Opening = opening;
            Closing = closing;
        ...
        public Encapsulator(EncapsulatorType type, char opening, char closing)
        {
            Type = type;
        ...
        public Encapsulator(string opening, string closing)
        {
            Type = null;
            Opening = opening;
            Closing = closing;
        ...
        public Encapsulator(object opening, object closing)
        {
            Type = null;
            Opening = opening.ToString();
            Closing = closing.ToString();
        ...
        public static implicit operator EncapsulatorType?(Encapsulator encapsulator)
        {
            return encapsulator.Type;
        }

        public static implicit operator string(Encapsulator encapsulator)
        {
            return encapsulator.Opening;
        }
    }

    public static List<Encapsulator> Encapsulators { get; } = new List<Encapsulator>()
    {
        new Encapsulator(EncapsulatorType.Parenthesis, '(', ')'),
        new Encapsulator(EncapsulatorType.Brackets, '[', ']'),
        new Encapsulator(EncapsulatorType.Braces, '{', '}'),
        new Encapsulator(EncapsulatorType.Chevrons, '<', '>')
    };

    public static bool EncapsulatorExists(char opening)
    {
        return Encapsulators.Exists(x => x.Opening == opening.ToString());
    }

    public static int GetEncapsulatorIndex(EncapsulatorType encapsulatorType)
    {
        return Encapsulators.FindIndex(x => x.Type == encapsulatorType);
    }

    public static string GetClosingEncapsulator(char opening)
    {
        int i = Encapsulators.FindIndex(x => x.Opening == opening.ToString());

        if (i > -1)
        {
            return Encapsulators[i].Closing;
        }
...
public static class Extensions
{
    ...
    public static string Encapsulate(this object obj, dynamic encapsulator)
    {
        string text = obj.ToString();
        string openingEncapsulator;
        string closingEncapsulator;

        if (encapsulator is EncapsulatorType || encapsulator is Encapsulator)
        {
            encapsulator = (encapsulator is EncapsulatorType) ? Encapsulators[GetEncapsulatorIndex(encapsulator)] : encapsulator;
            openingEncapsulator = encapsulator;
            closingEncapsulator = encapsulator.Closing;
        }
        else
        {
            ...
            if (RegexEncapsulateLiterals.IsMatch(openingEncapsulator))
            {
                ...
                foreach (char c in openingEncapsulator)
                {
                    if (!checknext)
                    {
                        ...
                        {
                            if (EncapsulatorExists(c))
                            {
                                closingEncapsulator = GetClosingEncapsulator(c) + closingEncapsulator;
                            }
                            else
                            {
